...
default_branch=$(gh api "repos/$tap_repository" --jq '.default_branch')
base_sha=$(gh api "repos/$tap_repository/commits/$default_branch" --jq '.sha')

if gh api "repos/$tap_repository/git/ref/heads/$branch" >/dev/null 2>&1; then
  echo "Refusing to replace existing tap branch $branch" >&2
  exit 1
fi

gh api "repos/$tap_repository/git/refs" --method POST \
  -f ref="refs/heads/$branch" \
  -f sha="$base_sha" >/dev/null

existing_sha=$(gh api "repos/$tap_repository/contents/$formula_path" -f ref="$branch" --jq '.sha' 2>/dev/null || true)
content=$(base64 < "$formula" | tr -d '\n')

if [ -n "$existing_sha" ]; then
  gh api "repos/$tap_repository/contents/$formula_path" --method PUT \
